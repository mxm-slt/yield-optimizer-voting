...
describe("VoterChefPool", function () {
  ...
  beforeEach(async function () {
    ...
    //   ["govt", this.ERC20Mock, ["GovT", "GovT", getBigNumber(10)]],
      ["rlp", this.ERC20Mock, ["LP", "rLPT", getBigNumber(10)]],
      ["rewardpool", this.RewardPool, []],
      ["r", this.ERC20Mock, ["Reward", "RewardT", getBigNumber(100000)]],
    ])

    await deploy(this, [["factory", this.VwaveFactory, [this.vwave.address, this.govt.address]]])
    this.chef = await this.MiniChefV2.attach(await this.factory.getChef())
    
    let voterAddress = await (await this.factory.newVoter(this.rewardpool.address)).wait()
    this.voter = await this.Voter.attach(voterAddress.events[5].args["voter"])
